...
def get_distance_matrix_response(coord_pairs, departure_time=None):
    ...
    for idx, r in enumerate(data['rows']):
        ...
        cols = ['start_lat','start_lng','end_lat','end_lng','start_adr','end_adr','dists', 'durs', 'status', 'departure_time']
...
def main():
    ...
    res.to_csv('%s/data/%s_training_data.csv' % (BASE_DIR, curr_date), index=False)
